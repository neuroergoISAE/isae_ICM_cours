...
Code based on the Matlab (Matlab sucks) implementation from authors of [1] (https://github.com/mnakanishi/TRCA-SSVEP)

Authors: Juan Jesús Torre, Ludovic Darmet
...
class TRCA(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X):
        """
        Make predictions on unseen data with the asociated probabilities. The new data observation X will be filtered
        with weights previously extracted and compared to the templates to assess
        similarity with each of them and select a class based on the maximal value.
        Parameters
        ----------
